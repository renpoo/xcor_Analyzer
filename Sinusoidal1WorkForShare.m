...
% --- WAVÇÃïîï™ì«Ç›çûÇ›ÅFsamplesÇÕäJénÇ∆èIóπÅiïbêî*FsÅAinfÇ≈ëSéûä‘Åj
samples = [1,inf];
...
if(1),
    ...
    % --- FilterÇ…ÇÊÇÈíxâÑÇÃämîF
    %grpdelay(df,2048,Fs); % Group delayÇ™àÍólÇ»ÇÁíPèÉÇ»éûä‘ÉVÉtÉgÇ…ÇÊÇËï‚ê≥
    D = mean(grpdelay(df)); % Average delay
    % --- Filter + íxâÑï‚ê≥èàóù
    slength = length(s);
    %s = filter(df, s); % íxâÑèàóùÇµÇ»Ç¢Filterèàóù
...
clear sCplx; % fftå„ÇÃï°ëfêîäiî[îzóÒ
clear sFreq; % fftå„ÇÃé¸îgêîäiî[îzóÒ
clear sMagn; % fftå„ÇÃã≠ìxÅiêUïùÅjäiî[îzóÒ
clear sPhas; % fftå„ÇÃà ëääiî[îzóÒ
for k=1:nframe,
...
% --- é¸îgêîóÃàÊÇÃëŒè∆ïîï™ÇÃîrèúÇ∆3D Graph plot
for tempk=1:nframe,
...
% --- MagnitudeÇ…ÇÊÇÈSortÇ∆Select
if(0),
    sortTop(1:nframe) = 8;
    clear sScplx; % Selectå„ÇÃï°ëfêîäiî[îzóÒ
    clear sSfreq; % Selectå„ÇÃé¸îgêîäiî[îzóÒ
    clear sSmagn; % Selectå„ÇÃã≠ìxÅiêUïùÅjäiî[îzóÒ
    clear sSphas; % Selectå„ÇÃà ëääiî[îzóÒ
    for tempk=1:nframe,
...
% --- 3D Graph plot
    if(0),
        for templ=1:sortTop,  % é¸îgêîé≤
            for tempk=1:nframe,  % éûä‘é≤
                t3D2(tempk) = (dftsize/Fs)*(tempk);
...
% --- MagnitudeÇ≈Sort&SelectÇµÇΩÇæÇØÇÃèÛë‘Ç≈IFFT
    if(0),
        ...
        for tempk=1:nframe,
            ...
            ifftS = horzcat( ifftS, real( ifft( Xp, dftsize )*5.5 ) ); %realÇëùïù
        end;
        sound( ifftS, Fs ); 
        plot(ifftS);
...
% --- FrequencyÇ…ÇÊÇÈSort
    clear sFcplx; % Frquency Sortå„ÇÃï°ëfêîäiî[îzóÒ
    clear sFfreq; % Frquency Sortå„ÇÃé¸îgêîäiî[îzóÒ
    clear sFmagn; % Frquency Sortå„ÇÃã≠ìxÅiêUïùÅjäiî[îzóÒ
    clear sFphas; % Frquency Sortå„ÇÃà ëääiî[îzóÒ
    for tempk=1:nframe,
...
% --- FrequencyÇ≈SortÇµÇΩÇæÇØÇÃèÛë‘Ç≈IFFT
    if(0),
        for tempk=1:nframe,
            for templ=1:sortTop,
...
% --- PeakåüèoÇ…ÇÊÇÈSelect
if(1),
    sortTop(1:nframe) = 0;
    tempStop = 0;
    clear sFcplx; % Selectå„ÇÃï°ëfêîäiî[îzóÒ
    clear sFfreq; % Selectå„ÇÃé¸îgêîäiî[îzóÒ
    clear sFmagn; % Selectå„ÇÃã≠ìxÅiêUïùÅjäiî[îzóÒ
    clear sFphas; % Selectå„ÇÃà ëääiî[îzóÒ
    for tempk=1:nframe,
        tempStop = 0;
...
% --- FrequencyÇ…ÇÊÇÈConnectîªíË
delta = 2^(1/12);
strcnt = 0;
clear sFstno; % ïRConnectîªíËå„ÇÃïRî‘çÜäiî[îzóÒ
sFstno = zeros(max(sortTop),nframe);
clear sCcplx; % ïRConnectå„ÇÃï°ëfêîäiî[îzóÒ
clear sCfreq; % ïRConnectå„ÇÃé¸îgêîäiî[îzóÒ
sCfreq = zeros(max(sortTop),nframe); % ïRConnectå„ÇÃé¸îgêîäiî[îzóÒ
clear sCmagn; % ïRConnectå„ÇÃã≠ìxÅiêUïùÅjäiî[îzóÒ
clear sCphas; % ïRConnectå„ÇÃà ëääiî[îzóÒ
if(1),
...
% --- ïRí∑Ç…ÇÊÇÈSort&SelectîªíË
clear sCstln; % ïRConnectå„ÇÃïRí∑
clear sLcplx; % ïRí∑Selectå„ÇÃï°ëfêîäiî[îzóÒ
clear sLfreq; % ïRí∑Selectå„ÇÃé¸îgêîäiî[îzóÒ
clear sLmagn; % ïRí∑Selectå„ÇÃã≠ìxÅiêUïùÅjäiî[îzóÒ
clear sLphas; % ïRí∑Selectå„ÇÃà ëääiî[îzóÒ
nString=strcnt;
strLen=13;
nSstring=0;
if(1),
...
% -----------------------------------------------
%âºÇ…Sortå„ÇÃëSÇƒÇÃäeçsÇ™Ç–Ç∆Ç¬ÇÃïRÇ≈Ç†ÇÈÇ∆Ç›Ç»Ç∑èàóù
%sLcplx = sFcplx;
%sLmagn = sFmagn;
%sLfreq = sFfreq;
%sLphas = sFphas;
...
% --- écë∂ïR(sLånóÒ)Ç…ëŒÇµÇƒIFFT&3éüï‚ä‘
if(1),
for templ=1:nSstring,
    for tempk=1:nframe,
...
if(1), % 3éüï‚ä‘èàóù
    ...
    % ïRÇâ¡éZÇµÇƒÇ¢Ç≠èàóù
    if(templ==1),
        sumS = ifftS;
    else
        sumS = sumS + ifftS;
    end;
end; % StringÇ≤Ç∆ÇÃèàóù
%sound( sumS, Fs ); 
